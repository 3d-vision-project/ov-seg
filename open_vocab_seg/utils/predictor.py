...
class OVSegPredictor(DefaultPredictor):
    def __init__(self, cfg, custom_config_path="configs/3dv_project.yaml"):
        super().__init__(cfg)
        self.custom_cfg = yaml.safe_load(open(custom_config_path))
        self.input_img_size = (self.custom_cfg['INPUT']['WIDTH'], self.custom_cfg['INPUT']['HEIGHT'])
